...
type ProjectCardProps = { title: string; description: string; tags: string[]; projectUrl: string; };
...
export function ProjectCard({ title, description, tags, projectUrl }: ProjectCardProps) {
  return (
    ...
    >
      <div className="aspect-video w-full bg-white/30" />
      <div className="flex flex-1 flex-col p-4 md:p-6">
        <h3 className="text-lg font-semibold tracking-tight">{title}</h3>
        <p className="mt-2 flex-1 text-sm text-muted-foreground">{description}</p>
        <div className="mt-4 flex flex-wrap gap-2">
          {tags.map((tag) => (
            <span key={tag} className="rounded-full bg-white/50 px-3 py-1 text-xs font-medium text-secondary-foreground">
              {tag}
            </span>
          ))}
        </div>
